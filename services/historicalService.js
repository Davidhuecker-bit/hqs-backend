...
async function getHistoricalPrices(symbol, period) {
  ...
  try {
    ...
    const backfilledCount = await withBackfillLock(sym, async () => {
      ...
      // Step 1: Flatfile bulk (best for older history, large date ranges)
      ...
      // Step 2: Grouped daily (best for recent trading days, no per-symbol quota)
      ...
      // Step 3: REST per-symbol range (fallback for specific symbol history)
